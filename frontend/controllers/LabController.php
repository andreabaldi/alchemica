...
class LabController extends Controller
{
    ...
    public function actionGenerateLutTest()
    {
        ...
        if ($model->load(Yii::$app->request->post())) {
            ...
            // Usiamo il metodo del modello per estrarre i parametri corretti (steps, invert, wedgeNumbers, etc.)
            $params = $model->getProcessingParams();

            try {
                $result = $engine->generateLutTestStrip($params);

                if ($result && isset($result['tiff'])) {
                    $path = Yii::getAlias('@frontend/web/uploads/targets/') . $result['tiff'];
                    return Yii::$app->response->sendFile($path, $result['tiff']);
                }
            } catch (\Exception $e) {
                Yii::$app->session->setFlash('error', "Errore Generazione: " . $e->getMessage());
            }
        }
