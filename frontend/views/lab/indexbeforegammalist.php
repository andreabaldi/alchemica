    ...
    <div class="d-flex justify-content-between align-items-center mb-4 border-bottom pb-3 bg-white p-3 rounded shadow-sm">
        <div>
            <h1 class="display-5 fw-bold text-dark mb-0">
                <span class="text-primary"><i class="fas fa-flask"></i> ALCHEMICH</span><span class="fw-light">LAB</span>
            </h1>
...
$this->registerJs("
    ...
    $('#preset-selector').on('change', function() {
        ...
        if (id && presetsData[id]) {
            $('#info-profile-name').text(presetsData[id].name);
            $('#info-gamma-base').text(presetsData[id].base);
            $('#info-gamma-delta').text('+' + presetsData[id].delta);
        }
        checkBoxVisibility();
    });
